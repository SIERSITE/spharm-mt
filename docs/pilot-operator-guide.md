...
|---|---|---|
| 2026-05-14 | rev21 | **Freeze de produção**. Adicionado: (i) banner de versão (`rev21 commit <git> built <iso>`) no início de cada execução do agent — diagnose multi-farmácia mais rápido. (ii) Comentário-contrato `SPHARM WRITE CONTRACT` em `spharm-orders-writer.ts` listando campos NÃO tocados (VVM_ID, Observacoes, Confirmado_*, Data Entrega, financeiros, B2B subsystem). (iii) `lib/feature-flags.ts` registry para kill switches. (iv) `/api/admin/pilot/summary` endpoint cross-tenant. (v) Build banner do SaaS em `/admin/pipeline`. (vi) `docs/production-freeze.md` + `docs/pilot-state-2026-05-14.md` baseline. |
